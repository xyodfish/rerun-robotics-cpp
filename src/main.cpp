...
static std::vector<VisualInstance> loadVisualsFromUrdf(
    ...
    for (const auto& go : geom_model.geometryObjects) {
        ...
        const std::string mesh_path = resolveMeshPath(go.meshPath, urdf_path, mesh_root_dir);
...
int main() {
    ...
    for (size_t i = 0; i < visuals.size(); ++i) {
        ...
        std::cout << "[INFO] mesh entity=" << vis.entity_path
                  << " parent_frame=" << vis.parent_frame_name << " verts=" << verts.size()
        ...
        if (fid >= data.oMf.size()) {
            std::cerr << "[WARN] invalid parent frame id for: " << vis.entity_path << std::endl;
            continue;
        }
        ...
        std::cout << "[INFO] transform entity=" << vis.entity_path << " t=[" << txyz.transpose()
                  << "]"
                  << " q=[" << qrot.x() << ", " << qrot.y() << ", " << qrot.z() << ", " << qrot.w()
                  << "]" << std::endl;
