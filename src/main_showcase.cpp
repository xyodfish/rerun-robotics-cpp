...
static rerun::Position3D toRerunPosition(const Eigen::Vector3d &p) {
  return rerun::Position3D(static_cast<float>(p.x()), static_cast<float>(p.y()),
                           static_cast<float>(p.z()));
}

static rerun::components::Vector3D toRerunVector(const Eigen::Vector3d &v) {
  return rerun::components::Vector3D(static_cast<float>(v.x()),
                                     static_cast<float>(v.y()),
                                     static_cast<float>(v.z()));
}

static rerun::components::LineStrip3D
toLineStrip(const std::vector<Eigen::Vector3d> &points) {
  std::vector<rerun::datatypes::Vec3D> rr_points;
  rr_points.reserve(points.size());
  for (const auto &p : points) {
    rr_points.emplace_back(static_cast<float>(p.x()), static_cast<float>(p.y()),
                           static_cast<float>(p.z()));
  }
...
static rerun::components::LineStrip3D
toLineStrip(const std::deque<Eigen::Vector3d> &points) {
  std::vector<rerun::datatypes::Vec3D> rr_points;
  rr_points.reserve(points.size());
  for (const auto &p : points) {
    rr_points.emplace_back(static_cast<float>(p.x()), static_cast<float>(p.y()),
...
static Eigen::Vector3d makeTargetPosition(double t) {
  return Eigen::Vector3d(0.55 + 0.12 * std::cos(1.3 * t),
...
static std::vector<rerun::components::Color> makeJointPalette() {
  return {
      rerun::components::Color(255, 99, 71),   rerun::components::Color(255, 165, 0),
      rerun::components::Color(255, 215, 0),   rerun::components::Color(50, 205, 50),
      rerun::components::Color(64, 224, 208),  rerun::components::Color(70, 130, 180),
...
int main() {
  ...
  rec.log_static(
      "plots/ee/error_norm",
      rerun::SeriesLines().with_names("ee_error_norm").with_colors(
          std::vector<rerun::components::Color>{
              rerun::components::Color(255, 99, 71)}));
  rec.log_static(
      "plots/ee/speed",
      rerun::SeriesLines().with_names("ee_speed").with_colors(
          std::vector<rerun::components::Color>{
  ...
  for (size_t i = 0; i < motions.size(); ++i) {
    ...
    rec.log_static(
        path,
        rerun::SeriesLines().with_names(motions[i].name.c_str()).with_colors(
            std::vector<rerun::components::Color>{color}));
  }
  ...
  for (int frame = 0;; ++frame) {
    ...
    if (!joint_positions.empty()) {
      ...
      rec.log("robot/debug/joint_chain/points",
              rerun::Points3D(rr_joint_points)
                  .with_colors(rr_joint_colors)
                  .with_radii(rr_joint_radii));

      rec.log("robot/debug/joint_chain/line",
              rerun::LineStrips3D(
                  std::vector<rerun::components::LineStrip3D>{
                      toLineStrip(joint_positions)})
                  .with_colors(std::vector<rerun::components::Color>{
                      rerun::components::Color(0, 220, 255)})
                  .with_radii(std::vector<float>{0.006f}));
    }

    rec.log("robot/debug/end_effector",
            rerun::Points3D(std::vector<rerun::Position3D>{toRerunPosition(ee_pos)})
                .with_colors(std::vector<rerun::components::Color>{
                    rerun::components::Color(0, 255, 127)})
                .with_radii(std::vector<float>{0.03f}));

    rec.log("robot/debug/target",
            rerun::Points3D(
                std::vector<rerun::Position3D>{toRerunPosition(target_pos)})
                .with_colors(std::vector<rerun::components::Color>{
                    rerun::components::Color(255, 69, 0)})
                .with_radii(std::vector<float>{0.03f}));
    ...
    if (target_trail.size() >= 2) {
      rec.log(
          ...
          rerun::LineStrips3D(std::vector<rerun::components::LineStrip3D>{
              toLineStrip(target_trail)})
              .with_colors(std::vector<rerun::components::Color>{
                  rerun::components::Color(220, 20, 60)})
              .with_radii(std::vector<float>{0.004f}));
    }

    rec.log("robot/debug/ee_velocity",
            rerun::Arrows3D::from_vectors(
                std::vector<rerun::components::Vector3D>{
                    toRerunVector(ee_vel * 0.08)})
                .with_origins(std::vector<rerun::Position3D>{
                    toRerunPosition(ee_pos)})
                .with_colors(std::vector<rerun::components::Color>{
                    rerun::components::Color(65, 105, 225)})
